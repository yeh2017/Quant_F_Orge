...
当前推荐 Windows 10/11 用户直接使用源码启动脚本，无需手动逐项安装依赖。

1. **下载**：从本仓库克隆项目，或 [直接下载 ZIP 源码包](https://github.com/yeh2017/QFO-Quant-Platform/archive/refs/heads/main.zip)。
2. **解压/放置**：建议放到不包含中文和空格的路径下，例如 `D:\QFO`。
3. **运行**：双击项目根目录内的 `run_first_time.bat`，等待后端和前端自动启动。
