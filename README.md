...
专业量化回测与数据分析平台，包含数据中台、因子选股、策略回测、组合优化、风险分析和新闻情绪分析等模块。

## 项目亮点

QFO 是一个面向 A 股量化分析的本地化平台，适合在个人电脑上完成数据同步、股票与 ETF 筛选、因子研究、策略回测、组合优化、风险分析、K 线可视化和新闻情绪跟踪。
...
| 数据中台 | 智能选股 |
| --- | --- |
| ![数据中台](assets/preview/data-center.png) | ![智能选股](assets/preview/stock-screener.png) |

| 策略回测 | 新闻中心 |
| --- | --- |
| ![策略回测](assets/preview/strategy-backtest.png) | ![新闻中心](assets/preview/news-center.png) |
...
| 模块 | 说明 |
| --- | --- |
| 数据中台 | 同步 A 股行情、ETF、可转债、财务数据、资金流向、融资融券、行业指数和事件数据。 |
| 智能选股 | 支持估值、成长、盈利、资金、行业、事件、技术趋势和新闻情绪等多条件筛选。 |
| 因子模块 | 提供价值、质量、动量、规模、低波、成长、红利、筹码集中等因子，并支持综合评分。 |
| 策略回测 | 支持多因子、MACD、布林带、网格、事件驱动等策略回测。 |
| 组合优化 | 支持最大夏普、最小方差、风险平价、等权配置等组合方案。 |
| 风险管理 | 计算 VaR、CVaR、最大回撤、Beta、Sortino 等风险指标。 |
| 可视化 | 查看 K 线图、资金流向、技术诊断和回测收益曲线。 |
| 新闻中心 | 聚合多源新闻，支持 LLM 情绪分析、事件识别、新闻自动抓取和摘要推送。 |
